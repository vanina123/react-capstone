...
import { getDisplays, initialState } from '../redux/HomeDisplay/homeSlice'
...
const Display = () => {
  ...
  const filter = Array.from(characters).filter((char) => char.name.toLowerCase().includes(input.toLowerCase()));

  const renderCharacters = () => {
    if (characters === initialState) {
      return (
      ...
      return (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-1 gap-4 lg:grid-cols-2 gap-4 p-6">
          {filter.map((char) => (
            <Link className="links" to={`/details/${char.id}`} element={<Details key={char.id} />} key={char.id}>
              <CardDisplay char={char} />
            </Link>
          ))}
          {filter.length === 0 && (
